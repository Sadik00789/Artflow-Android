...
class TransformerNetTF(tf.Module):
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=[1, 1024, 1024, 3], dtype=tf.float32, name="input")])
    def __call__(self, x):
...
class AnimeGANGeneratorTF(tf.Module):
    ...
    Conforms to ArtFlow Graph Contract:
    Input: [1, 512, 512, 3] float32 in [0.0, 255.0]
    Output: [1, 512, 512, 3] float32 in [0.0, 255.0]
    """
    def __init__(self, pt_checkpoint_path: str):
        super().__init__()
